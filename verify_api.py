...
from donations.models import Categories, Donations, Transactions
...
def run_verification():
    ...
    donor_user, _ = Users.objects.get_or_create(
        email="donor@test.com",
        username="donor",
        defaults={"full_name": "Donor User", "role": role},
    )
    ...
    response = client.post(
        "/auth/jwt/create/", {"email": "donor@test.com", "password": "password123"}
    )
    ...
    response = client.post(
        f"/api/donations/{donation.id}/save/", {"donation_id": donation.id}
    )  # Using URL param or body? View expects body for create, but URL for save-donation endpoint might be confusing.
